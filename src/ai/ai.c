...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	...
	// Process nodes while priority queue is not empty
	while (pq.count > 0) {
		...
		// Record node in array
		update_explore_table(n, expanded_nodes);

		// If found a solution save solution
		...
		// For each move action LRUD in that order
		move_t moves[4] = {left, right, up, down};
		for (int i = 0; i < 4; i ++) {
			// Create Child new_node and check is sucessfull move
			node_t* new_node = NULL;
			bool player_moved = applyAction(init_data, n, &new_node, moves[i]);

			generated_nodes++;
			
			// If not sucessfull move or have deadlock not in solution
			if (!player_moved || simple_corner_deadlock(init_data, &(n -> state))) {
				free(new_node);
				continue;
			}

			// Convert map to a 1D map
			flatten_map(init_data, &(flat_map) , new_node -> state.map);
			
			// If have duplicates in hashtable, don't have to add to hashtable
			...
			// Insert flatmap into hashtable if passed previous checks
